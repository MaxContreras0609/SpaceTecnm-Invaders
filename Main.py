...
enemy_y_change = []
num_of_enemies = 3
enemy_speed = 0.5  # Reducir la velocidad de los enemigos
...
# Función para verificar si un enemigo toca al jugador
def is_game_over(enemy_x, enemy_y, player_x, player_y):
    for i in range(num_of_enemies):
        if enemy_y[i] > 440 and (enemy_x[i] in range(player_x, player_x + 60)):  # El enemigo toca al jugador
            return True
    return False
...
while running:
    ...
    for i in range(num_of_enemies):
        ...
        # Comprobar colisiones con la bala
        collision = is_collision(enemy_x[i], enemy_y[i], bullet_x, bullet_y)
        if collision:
            bullet_y = 480
            bullet_state = "ready"
            score += 1  # Incrementar puntuación solo cuando la bala toca al enemigo
    ...
    # Verificar si el juego termina por colisión con el jugador
    if is_game_over(enemy_x, enemy_y, player_x, player_y):
